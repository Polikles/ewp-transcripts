...
| `GUI_REVIEW_MARKER_REQUIRED` | Separating text was requested without exactly one `[[marked text]]` group containing complete words or a sentence. | The current block remains unchanged and no markers are saved. | Wrap the intended words or sentence in `[[double brackets]]`, then select **Separate marked text**. |
