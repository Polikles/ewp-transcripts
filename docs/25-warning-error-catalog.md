...
| `GUI_PATH_REJECTED` | An input/output path is missing, a symlink, the wrong type, or in a prohibited operating-system directory. | The GUI prevents accidental operation on system trees while accepting ordinary user-space paths. | Select a real non-symlink path outside system directories. |
| `GUI_FILESYSTEM_REQUEST_INVALID` | The legacy local path browser received a missing, unreadable, symlinked, unsupported-filter, or out-of-root path. | That constrained browser cannot enumerate outside its supplied search locations or follow symlinks. | Use a direct user-space path or a browser-native result picker; report a needed picker workflow. |
...
| `GUI_MANAGED_SOURCE_REQUEST_INVALID` | Managed-source inventory or cleanup omitted a valid shared output directory, workspace catalog, selection, or confirmation. | No durable source copy is removed. | Select the exact shared output directory, refresh the inventory, select eligible copies, and confirm cleanup. |
| `GUI_MANAGED_SOURCE_CLEANUP_REFUSED` | A selected managed copy is referenced, changed, unsafe, or no longer matches the freshly audited inventory. | Cleanup fails closed before deleting the selected set. | Refresh the inventory and retain referenced copies; remove queue/workspace references only when that work is intentionally retired. |
| `GUI_SOURCE_FINGERPRINT_MISMATCH` | A queued source no longer has the exact SHA-256 fingerprint inspected during dry-run. | The queue fails before opening the changed media for transcription. | Inspect the current file again, review the new dry-run, and stage it as a new job. |
...
| `GUI_QUEUE_ITEM_IMMUTABLE` | The single-row staging control targeted a missing item or one that has already started. | Running work cannot be deleted through the staging control. | Remove a staged row, or use the confirmed selected-item control for inactive history. |
| `GUI_QUEUE_REMOVE_INVALID` | Selected-item removal lacked confirmation or a valid unique list of queue IDs. | No queue state or files are changed. | Select at least one inactive row, confirm the warning, and retry. |
| `GUI_QUEUE_ACTIVE` | A clear/remove request included queued or running transcription. | Active GPU work is not interrupted or forgotten. | Let active transcription finish before clearing/removing it. |
| `GUI_WORKFLOW_ERROR_INVALID` | The browser attempted to record an unsupported workflow stage or malformed error code. | Queue state cannot be poisoned by arbitrary browser content. | Retry the supported operation from the current GUI page. |
| `GUI_WORKFLOW_SKIP_INVALID` | The browser attempted to skip a required stage or an unknown workflow stage. | Only optional LLM correction and translation may be explicitly skipped. | Use the offered skip control for correction or translation only. |
| `GUI_WORKFLOW_OPERATION_FAILED` | The frontend could not identify a narrower code for a later workflow operation. | The affected queue stage is marked failed rather than implied complete. | Inspect the displayed operation error and retry deliberately. |
...
| `GUI_REVIEW_SELECTION_REQUIRED` | Separating text was requested without a nonempty selection of complete words or a sentence. | The current block remains unchanged. | Select the intended words or sentence, including any desired punctuation, then select **Separate selected text**. A leading `.`, `!`, or `?` immediately before a selected new sentence is accepted and remains with the preceding sentence. |
...
| `GUI_REVIEW_RESTORE_FAILED` | The browser remembered a review whose result or review file is no longer readable or is prohibited. | The stale pointer is discarded; no artifact is changed. | Restore access to the user-space path, or prepare the existing saved review again. |
...
| `GUI_REVIEW_SESSION_INPUT_REQUIRED` | Restore was requested without both a canonical result and review output root. | The GUI cannot select a saved draft ambiguously. | Choose the canonical result and its persistent review root, then restore. |
...
| `GUI_REVIEW_FILE_MISSING` | A canonical result, review, revision, or other file required by transcript review was removed or moved. | Review stops before publishing from incomplete lineage. | If the managed canonical copy was removed, add the original `*_results.json` to a workflow queue again; otherwise restore the named file from backup. |
...
| `GUI_CORRECTION_REQUEST_INVALID` | Correction paths or numeric/request fields are malformed or prohibited. | No provider request is made from ambiguous input. | Correct the displayed fields and select ordinary user-space locations. |
...
| `GUI_TRANSLATION_CLOUD_OPT_IN_REQUIRED` | OpenRouter translation was selected without explicit cloud opt-in. | The local-first policy cannot be bypassed implicitly. | Select cloud operation knowingly and confirm disclosure. |
...
| `GUI_TRANSLATION_PROVIDER_INVALID` | The GUI submitted an unsupported translation provider. | No ambiguous endpoint is contacted. | Select LM Studio or OpenRouter. |
| `GUI_TRANSLATION_MODEL_REQUIRED` | No exact translation model ID was supplied. | The backend cannot choose a model implicitly. | Enter the exact ID advertised by the selected provider. |
| `GUI_TRANSLATION_MODEL_UNAVAILABLE` | The selected backend does not advertise the exact translation model. | Work cannot silently run on another model. | Load/select the exact advertised model ID. |
| `GUI_TRANSLATION_REASONING_INVALID` | The OpenRouter reasoning-token budget is malformed or negative. | The provider request cannot change reasoning behavior implicitly. | Enter a non-negative whole number. |
...
| `GUI_TRANSLATION_CREDENTIAL_MISSING` | No OpenRouter key is available to the active GUI server process. | Browser or project storage is not searched for credentials. | Set a session-only key or the configured server environment variable. |
| `GUI_TRANSLATION_CREDENTIAL_REJECTED` | OpenRouter rejected the active key or returned invalid key metadata. | Authentication failure is distinct from model availability. | Check the key, provider account, and permissions. |
| `GUI_TRANSLATION_BACKEND_REJECTED` | The translation endpoint returned a non-authentication HTTP error during readiness. | A reachable but rejecting service is not labeled offline. | Read the status and check the provider or endpoint. |
| `GUI_TRANSLATION_BACKEND_UNAVAILABLE` | The translation backend did not pass the bounded readiness check. | Source text is not sent while the backend is unavailable. | Start/check the backend and endpoint, then retry. |
...
| `GUI_TRANSLATION_REQUEST_INVALID` | Translation paths, dictionary, endpoint, or request fields are invalid. | No candidate is published from ambiguous input. | Correct the displayed fields and select ordinary user-space locations. |
| `GUI_TRANSLATION_RESULT_REQUIRED` | Translation skip was requested without a canonical result. | The queue cannot record a skip against an unknown item. | Choose a completed canonical result first. |
| `GUI_TRANSLATION_CANDIDATE_NOT_FOUND` | The queue has no non-final translation candidate to reopen for semantic review. | The GUI does not guess a candidate or create a final translation. | Generate or select the intended candidate, then open semantic review. |
...
| `GUI_TRANSLATION_REVIEW_SAVE_REQUIRED` | Preview was selected while editable target changes were still unsaved. | Preview evidence always describes the exact review file that may later be applied. | Save the translation draft, then preview it. |
| `GUI_TRANSLATION_REVIEW_RESTORE_FAILED` | A browser-saved translation-review pointer cannot be read or validated. | The stale pointer is discarded; no translation artifact changes. | Reopen the intended candidate or prepare its semantic review again. |
...
| `GUI_TRANSLATION_REVIEW_PREVIEW_REQUIRED` | Apply was requested without previewing the exact saved translation review with the current empty-unit setting. | Unvalidated target text or a changed omission policy cannot become final. | Save, choose whether intentional empty units are allowed, preview, inspect, confirm, then apply without changing that setting. |
...
| `GUI_CORRECTION_RESULT_REQUIRED` | Correction skip was requested without a canonical result. | The queue cannot record a skip against an unknown item. | Choose a completed canonical result first. |
| `GUI_REVIEW_SPEAKER_IN_USE` | A revision-only speaker was removed while still assigned to visible review text. | Existing text is not silently reassigned. | Reassign or merge the speaker's blocks, then remove the unused speaker. |
| `GUI_TRANSCRIPTION_OPTIONS_INVALID` | The GUI received an unsupported language or speaker-count setting. | Dry-run and transcription cannot disagree about their settings. | Select an offered language and automatic or 1–6 speakers. |
| `GUI_API_UNAVAILABLE` | The browser cannot reach or negotiate with the local GUI API. | No operation result is assumed. | Verify that `transcriber gui` is still running and reload the printed loopback URL. |
| `GUI_API_VERSION_MISMATCH` | Browser assets and the local GUI API advertise incompatible versions. | The frontend stops rather than sending mismatched requests. | Refresh after updating the repository; restart the GUI if needed. |
