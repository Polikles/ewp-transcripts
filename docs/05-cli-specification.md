...
```text
transcriber gui [--port PORT] [--no-open-browser]
```

Starts the local browser adapter on `127.0.0.1`. Port `8765` is the default and `0` requests
an ephemeral port. User-space paths are available without a launch-time allowlist; operating-
system directories and symlinks are rejected. The command serves bundled assets and the
